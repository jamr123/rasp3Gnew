...
class Data:
    ...
    def __init__(self):
        ...
        GPIO.add_event_detect(config.CAUDAL_SENSOR_1_GPIO, GPIO.FALLING, callback=self.ISR_caudal_1, bouncetime=1500)
        
        GPIO.setup(config.CAUDAL_SENSOR_2_GPIO, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.add_event_detect(config.CAUDAL_SENSOR_2_GPIO, GPIO.FALLING, callback=self.ISR_caudal_2, bouncetime=1500)
